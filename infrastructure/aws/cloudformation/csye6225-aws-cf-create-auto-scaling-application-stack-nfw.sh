...
aws cloudformation create-stack --stack-name ${stackname} --template-body file://./csye6225-cf-auto-scaling-application-nfw.json --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey=vpcId,ParameterValue=$vpcid ParameterKey=ImageId,ParameterValue=$ami ParameterKey=publicsubnet1,ParameterValue=$PublicSubnet1 ParameterKey=publicsubnet2,ParameterValue=$PublicSubnet2 ParameterKey=publicsubnet3,ParameterValue=$PublicSubnet3 ParameterKey=circleci,ParameterValue=$UserName ParameterKey=s3bucket,ParameterValue=$s3bucket ParameterKey=Domain,ParameterValue=$Domain ParameterKey=LoadBalancerName,ParameterValue=$LoadBalancerName ParameterKey=CertificateArn1,ParameterValue=$CERTIFICATE_ARN
